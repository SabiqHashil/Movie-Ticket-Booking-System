        ...
        public async Task<List<Movie>> GetMoviesAsync()
        {
            ...
            try
            {
                using (var conn = new SqlConnection(GetConnectionString()))
                {
                    ...
                    //using (var cmd = new SqlCommand("SELECT * FROM Movies", conn))
                    {
        ...
        public List<Schedule> GetAvailableSchedules(int movieID, int? scheduleID = null)
        {
            ...
            using (SqlConnection connection = new SqlConnection(GetConnectionString()))
            {
                using (SqlCommand command = new SqlCommand("sp_MovieBooking", connection))
                {
                    ...
                    // Add parameters based on inputs
                    command.Parameters.AddWithValue("@MovieID", movieID);

                    // Add optional ScheduleID parameter only if provided
            ...
            using (SqlConnection conn = new SqlConnection(GetConnectionString()))
            {
                using (SqlCommand cmd = new SqlCommand("sp_InsertTicket", conn))
                {
                    ...
                    try
                    {
                        ...
                        // Retrieve the generated TicketNumber from the output parameter
